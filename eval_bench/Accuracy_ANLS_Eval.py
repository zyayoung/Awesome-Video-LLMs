    ...
    def evaluate(self, quesIds=None):
        ...
        def eval_pred_list(preds, gts, quesIds):
            ...
            for quesId in quesIds:
                ...
                pred_answer = self.answer_processor(preds[quesId])
                unique_answer_scores = _compute_answer_scores(gts[quesId])
            ...
            if len(pred_scores) == 0:
                print("{}###{}###{}".format(preds, gts, quesIds))
                accuracy = 0
            else:
        ...
        if self.datatype in accuracy_metric_tasks:
            quesIds = list(gts.keys())
            return round(eval_pred_list(self.vqaRes, self.vqa, quesIds),5)

        if self.datatype in ANLS_metric_tasks:
            import editdistance
            score = 0
            ...
            for quesId in quesIds:
                ...
                self.answer_processor(res[quesId])
                gts[quesId] = [self.answer_processor(ansDic) for ansDic in gts[quesId]]
                resAns = self.answer_processor(res[quesId])
                best_answer = [0.0]
